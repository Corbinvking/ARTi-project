...
-- This allows semantic search across campaigns, playlists, and other content

-- Add embedding column to campaigns table for vector search
ALTER TABLE public.campaigns ADD COLUMN IF NOT EXISTS embedding vector(1536);
...
CREATE TABLE IF NOT EXISTS public.content_embeddings (
    ...
    CONSTRAINT content_embeddings_content_id_fkey FOREIGN KEY (content_id) REFERENCES public.campaigns(id) ON DELETE CASCADE
);
...
CREATE POLICY "Users can view content embeddings for accessible content" ON public.content_embeddings
    FOR SELECT USING (
        content_type = 'campaign' AND EXISTS (
            SELECT 1 FROM public.campaigns c WHERE c.id = content_id AND c.org_id = auth.jwt() ->> 'org_id'
        )
    );
