...
export async function providersRoutes(fastify: FastifyInstance) {
  ...
  fastify.get('/providers/connections', {
    ...
    try {
      ...
      request.log.error(error, 'Error fetching provider connections')
  ...
  fastify.get('/providers/spotify/callback', async (request: FastifyRequest, reply: FastifyReply) => {
  ...
  fastify.post('/providers/spotify/sync', {
    ...
    try {
      ...
      request.log.error(error, 'Error queueing Spotify sync job')
  ...
  fastify.get<{ Querystring: MetricsRequest['Querystring'] }>('/metrics/:platform', {
    preHandler: [fastify.requireAuth]
