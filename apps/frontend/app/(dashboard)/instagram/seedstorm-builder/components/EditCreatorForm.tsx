...
import { Creator, MUSIC_GENRES, CONTENT_TYPES, COUNTRIES } from "../lib/types";
import { updateCreator } from "../lib/localStorage";
...
export const EditCreatorForm = ({ creator, onSuccess }: EditCreatorFormProps) => {
  const [formData, setFormData] = useState({
    ...
    reel_rate: creator.reel_rate.toString(),
    carousel_rate: creator.carousel_rate?.toString() || '',
    story_rate: creator.story_rate?.toString() || ''
  });

  const [selectedGenres, setSelectedGenres] = useState<string[]>(creator.music_genres);
  const [selectedContentTypes, setSelectedContentTypes] = useState<string[]>(creator.content_types);
  const [selectedCountries, setSelectedCountries] = useState<string[]>(
    creator.audience_countries?.filter(c => c !== creator.base_country) || []
  );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Fix: updateCreator expects 2 arguments (id, updates)
    await updateCreator(updatedCreator.id, updatedCreator);
    ...
    toast({
      ...
      description: `@${updatedCreator.instagram_handle} has been updated`,
    });
    onSuccess();
  };
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="space-y-2">
          <Label htmlFor="median_views_per_video">Recent Median Views</Label>
          <Input
